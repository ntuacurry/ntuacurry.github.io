...
def load_data(symbol):
    """
    下載股票資料。
    如果 symbol 是 '^TWOII'，則從 Google Sheet 讀取；
    否則使用 yfinance 下載。
    """
    # ----------------------------------------------------
    # 新增邏輯：針對櫃買指數 (^TWOII) 讀取 Google Sheet
    # ----------------------------------------------------
    if symbol == '^TWOII':
        try:
            url = "https://docs.google.com/spreadsheets/d/e/2PACX-1vRxAhYyyPNAgvSGDDfFUM36dqwIC4KCxysWibJRyn7zvqiz-d351uaNNV7DekJiO58q4YrueFU_Sg4v/pub?gid=0&single=true&output=csv"
            
            # 讀取 CSV
            df = pd.read_csv(url)
            
            # 1. 欄位更名：將中文對應到程式需要的英文欄位
            # 根據您的截圖：日期(A), 開盤指數(B), 最高指數(C), 最低指數(D), 收盤指數(E)
            df.rename(columns={
                '日期': 'Date',
                '開盤指數': 'Open',
                '最高指數': 'High',
            ...
            # 2. 轉換日期格式
            # 原始格式看起來是 20130305 (YYYYMMDD 的數字或字串)
            df['Date'] = df['Date'].astype(str) # 確保是字串
            ...
            # 4. 補齊缺失欄位 (避免程式後面報錯)
            # Google Sheet 中似乎沒有成交量，但後續程式會過濾 Volume > 0
            # 所以這裡塞一個假數據 (例如 1000000) 讓它通過過濾
            df['Volume'] = 1000000.0 
            
            # 指數通常沒有配息或分割，設為 0
            df['Dividends'] = 0.0
            df['Stock Splits'] = 0.0
            df['Adj Close'] = df['Close'] # 假設無 Adj Close 則用 Close 代替

            df.sort_index(inplace=True)
            return df
            
        except Exception as e:
            st.error(f"讀取櫃買指數 Google Sheet 失敗: {e}")
            return pd.DataFrame()
